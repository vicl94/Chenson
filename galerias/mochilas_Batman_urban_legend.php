...
$consulta=mysql_query("select * from tienda where cuerpo= 'mochila' and marca='Batman urban legend'", $conexion);
...
if (is_numeric($nro_pagina)){
	...
	$consulta=mysql_query("select * from tienda  where cuerpo= 'mochila' and marca='Batman urban legend' limit $inicio,$reg_por_pagina",$conexion);
  ...
  fjs.parentNode.insertBefore(js, fjs);
	...
	<!--termina plugin facebook-->
        ...
        <div class= "titulos2">Mochilas</div>
				<div class="sub">Batman urban legend</div>
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='mochilas_Batman_urban_legend.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='mochilas_Batman_urban_legend.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='mochilas_Batman_urban_legend.php?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
