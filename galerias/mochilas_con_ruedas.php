<html>
	<title>Mochilas con ruedas |Mochilas Chenson</title>
...
$consulta=mysql_query("select * from tienda where cuerpo= 'mochila con ruedas' ", $conexion);
...
if (is_numeric($nro_pagina)){
	...
	$consulta=mysql_query("select * from tienda  where cuerpo= 'mochila con ruedas' limit $inicio,$reg_por_pagina",$conexion);
	$can_paginas=ceil ($nro_reg/$reg_por_pagina);
	
	
	$sql = ("SELECT * from tienda where cuerpo = 'mochila con ruedas' and tamano = 'chico'");
	$result = mysql_query($sql);
    $numero = mysql_num_rows($result);
	
	$sqlGde = ("SELECT * from tienda where cuerpo = 'mmochila con ruedas' and tamano = 'grande'");
...
<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
	...
	<!--termina plugin facebook-->
        ...
        <div id="menuTam">
        <center>
            <h4>Busqueda por tamaño</h4>
        </center>    
            <br />
            <a href="mochilascon_ruedas_chicas.php" class="clase2">°Chica (<?php echo $numero ?>)</a> 
            <br /><br />
            <a href="mochilas_ruedas_grandes.php" class="clase2">°Grande (<?php echo $numeroGde ?>)</a> 
            <br /><br />
        </div>
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='mochilas_con_ruedas.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='mochilas_con_ruedas.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='mochilas_con_ruedas.php?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
