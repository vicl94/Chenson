...
$consulta=mysql_query("select * from tienda where cuerpo= 'mochila de camping' and tamano='camping'", $conexion);
...
if (is_numeric($nro_pagina)){
	...
	$consulta=mysql_query("select * from tienda  where cuerpo= 'mochila de camping' and tamano ='camping'limit $inicio,$reg_por_pagina",$conexion);
...
<script>(function(d, s, id) {
  ...
  <!--Termina buscador-->
		...
		<div class="contenedor">
			...
			<div class="caja">
            	<!--<img src="<?php echo $imgPl;?>" width="35px" align="right" title="<?php echo $Plus ?>"/>-->
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='mochilas_de_camping.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='mochilas_de_camping.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='mochilas_de_camping.php?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
