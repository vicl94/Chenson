<html>
	<title>Fundas para laptop |Mochilas Chenson</title>
...
$consulta=mysql_query("select * from tienda where cuerpo= 'funda de neopreno' and tamano = 'mediano'", $conexion);
...
if (is_numeric($nro_pagina)){
	...
	$consulta=mysql_query("select * from tienda  where cuerpo= 'FUNDA DE NEOPRENO' and tamano = 'mediano' limit $inicio,$reg_por_pagina",$conexion);
	$can_paginas=ceil ($nro_reg/$reg_por_pagina);
	
	
	$sql = ("SELECT * from tienda where cuerpo = 'FUNDA DE NEOPRENO' and tamano = 'chico'");
	$result = mysql_query($sql);
    $numero = mysql_num_rows($result);
	
	$sqlMed = ("SELECT * from tienda where cuerpo = 'FUNDA DE NEOPRENO' and tamano = 'mediano'");
	$resultMed = mysql_query($sqlMed);
    $numeroMed = mysql_num_rows($resultMed);
	
	$sqlGde = ("SELECT * from tienda where cuerpo = 'FUNDA DE NEOPRENO' and tamano = 'grande'");
  ...
  fjs.parentNode.insertBefore(js, fjs);
	...
	<!--termina plugin facebook-->
    	...
    	<header>
         ...
         <!--galeria-->
        <div class= "titulos2">Funda para laptop</div>
        	...
        	<form form name="prueba" action="#" method="post" >
                ...
                return false;' style="adjuntar etiqueta de titulo utilizando el atributo &quot;for&quot;">
                    <option selected="selected">Filtrar por tamaño</option>	
                     <option value="funda_para_laptop_chica.php">Chico (<?php echo $numero ?>)</option></a>
                      <option value="funda_para_laptop_mediana.php">Mediano (<?php echo $numeroMed ?>)</option>
                      <option value="funda_para_laptop_grande.php">Grande (<?php echo $numeroGde ?>)</option>
                </select></form>
		</div>
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='funda_para_laptop_mediana.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='funda_para_laptop_mediana.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='funda_para_laptop.php_mediana?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
