...
$consulta=mysql_query("select * from tienda where cuerpo= 'mochila' and Plus= 'Portalaptop' and tamano='mediano'", $conexion);
...
if (is_numeric($nro_pagina)){
	...
	$consulta=mysql_query("select * from tienda  where cuerpo= 'mochila' and Plus= 'Portalaptop' and tamano='mediano' limit $inicio,$reg_por_pagina",$conexion);
	...
	$sqlMed = ("SELECT * from tienda where cuerpo = 'mochila' and tamano = 'mediano' and plus ='portalaptop'");
	$resultMed = mysql_query($sqlMed);
    $numeroMed = mysql_num_rows($resultMed); 


	$sqlGde = ("SELECT * from tienda where cuerpo = 'mochila' and tamano = 'grande' and plus ='portalaptop'");
  ...
  fjs.parentNode.insertBefore(js, fjs);
	...
	<!--termina plugin facebook-->
    	...
    	<header>
         ...
         <!--galeria-->
        <div class= "titulos2">Mochila Portalaptop</div>
				<div class="sub">Mediana</div>
        	...
        	<form form name="prueba" action="#" method="post" >
                ...
                return false;' style="adjuntar etiqueta de titulo utilizando el atributo &quot;for&quot;">
                    <option selected="selected">Filtrar por tamaño</option>	
                      <option value="mochila portalaptop mediana.php">Mediano (<?php echo $numeroMed ?>)</option>
                      <option value="mochila portalaptop grande.php">Grande (<?php echo $numeroGde ?>)</option>
                </select></form>
		</div>
        <!--prueba filtrar tamaño-->
       <!--Regresar principal-->
         <div class="RegPcpl2">
         	...
         	<form action="#" method="post" name="Filtrar"><input name="Portalaptop" type="radio" value="mochilas.php" checked="checked" /><a href="mochila portalaptop.php" class="clase4">Regresar a mochilas portalaptop</a></form>
         </div>
         <!--Regresar principal-->
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='mochila portalaptop mediano.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='mochila portalaptop mediano.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='mochila portalaptop mediano.php?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
