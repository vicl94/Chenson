...
$consulta=mysql_query("select * from tienda where cuerpo= 'mochila' and tamano='mediano'", $conexion);
	...
	<!--termina plugin facebook-->
     ...
     <!--   <div class="filtroLic">
       ...
       <!--Regresar principal-->
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='mochilas_medianas.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='mochilas_medianas.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='mochilas_medianas.php?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
