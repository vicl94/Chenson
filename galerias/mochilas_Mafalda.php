...
$consulta=mysql_query("select * from tienda where cuerpo= 'mochila' and marca = 'Mafalda'", $conexion);
...
if (is_numeric($nro_pagina)){
	...
	$consulta=mysql_query("select * from tienda  where cuerpo= 'mochila' and marca = 'Mafalda' limit $inicio,$reg_por_pagina",$conexion);
...
<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
	...
	<!--termina plugin facebook-->
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='mochilas_Mafalda.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='mochilas_Mafalda.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='mochilas_Mafalda.php?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
