<html>
	<title>Sandichos |Mochilas Chenson</title>
    <link rel="shortcut icon" type="image/x-icon" href="/../img/icons/chenson.ico">
</html>
<?php 
include ('../conexion.php');
require_once('getPrice.php');
$consulta=mysql_query("select * from tienda where marca= 'sandichos'", $conexion);
...
if (is_numeric($nro_pagina))
	...
	$consulta=mysql_query("select * from tienda  where marca= 'sandichos' limit $inicio,$reg_por_pagina",$conexion);
...
$(function () {
 ...
 <!--fixed-->
    ...
    <body>
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='sandichos.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='sandichos.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='sandichos.php?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
