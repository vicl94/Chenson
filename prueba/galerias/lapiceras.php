<html>
	<title>Portalapices |Mochilas Chenson</title>
    <link rel="shortcut icon" type="image/x-icon" href="/../img/icons/chenson.ico">
</html>
<?php 
include ('../conexion.php');
require_once('getPrice.php');
$consulta=mysql_query("select * from tienda where cuerpo= 'lapicera'", $conexion);
...
if (is_numeric($nro_pagina))
	...
	$consulta=mysql_query("select * from tienda  where cuerpo= 'lapicera' limit $inicio,$reg_por_pagina",$conexion);
 ...
 <!--fixed-->
    ...
    <body>
    	...
    	<header>
         ...
         <!--galeria-->
         <div class= "titulos2">Lapiceras</div>
        ...
        <?php 
		if($nro_pagina>1)
			echo"<a href='lapiceras.php?num=".($nro_pagina-1)."'> Anterior</a>";
			
			for($i=1; $i<=$can_paginas;$i++){
				if ($i==$nro_pagina)
					echo $i."";
				else
					echo"<a href='lapiceras.php?num=$i'>$i</a>";
				}
		if($nro_pagina<$can_paginas)		
			echo"<a href='lapiceras.php?num=".($nro_pagina+1)."'>Siguiente</a>";		
		?>
        </div>
         <!--termina galeria-->
